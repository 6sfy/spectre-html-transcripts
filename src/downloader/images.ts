...
export class TranscriptImageDownloader {
  private static log = debug('spectre-html-transcripts:TranscriptImageDownloader');
  ...
  withCompression(quality = 80, convertToWebP = false, options: Omit<WebpOptions, 'quality' | 'force'> = {}) {
    ...
    import('sharp').catch((err) => {
      console.error(err);
      console.error(
        `[spectre-html-transcripts] Failed to import 'sharp'. Image compression requires the 'sharp' package to be installed. Either install sharp or remove the compression options.`
      );
    });
  ...
  build(): ResolveImageCallback {
    return async (attachment) => {
      ...
      const requestOptions: Record<string, any> = {}; // typed instead of any
      if (this.token) requestOptions.headers = { authorization: this.token };
      const response = await request(attachment.url, requestOptions).catch((err: unknown) => {
      ...
      const buffer = await response.body.arrayBuffer().then((res: ArrayBuffer) => Buffer.from(res)); // typed res
