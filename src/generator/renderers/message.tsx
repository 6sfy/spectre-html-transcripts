...
import { type Message as MessageType } from 'discord.js-selfbot-v13';
...
export default async function DiscordMessage({
  ...
  return (
    ...
    >
      ...
      {/* message embeds */}
      {message.embeds.map((embed: any, id: number) => (
      ...
      {message.components.length > 0 && (
        <DiscordAttachments slot="components">
          {message.components.map((component: any, id: number) => (
      ...
      {message.reactions.cache.size > 0 && (
        <DiscordReactions slot="reactions">
          {message.reactions.cache.map((reaction: any, id: number) => (
            <DiscordReaction
              key={`${message.id}r${id}`}
