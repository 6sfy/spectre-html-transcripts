...
If you want to provide your own messages for finer control of what `spectre-html-transcripts` will save, use this function.
...
// By default returns a MessageAttachment that can be sent in a channel.
channel.send({
    files: [attachment]
});
```
...
```typescript
import * as spectreTranscripts from "spectre-html-transcripts";
...
An object with the spectre-html-transcripts configuration options.
...
- attachment: the HTML data as a `MessageAttachment`
...
Disabling this will remove the `Powered by spectre-html-transcripts` in the footer.
